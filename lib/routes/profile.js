...
module.exports = router
  ...
  .put('/', (req, res, next) => {
    // I don't think this work is necessary.
    // #1 Mongoose won't set fields that are not defined on the model.
    // #2 Mongoose defines findByIdandUpdate as a `$set`, so you will only 
    // update fields that have been provided anyway
    // #3 Instead, explicitly delete fields from req.body that are not allowed but
  ...
  .put('/friends/confirm/:id', (req, res, next) => {
    ...
    // Unfortunately, this route allows caller to add anyone as a friend 
    // (and add myself to their friends) regardless of pending state:(
      // return Promise.all([
      ...
      //   .then(([,user]) => res.json(user))
      //   .catch(next);
    // Need to check: does this user have a pending friend request from this user?
    User.find({ _id: userId, pendingFriends: friendId })
      .count()
      .then(count => {
        if(!count) throw { status: 400, error: 'No pending friend request' };
      })
      .then(() => {
        return Promise.all([
          User.findByIdAndUpdate(userId, {
