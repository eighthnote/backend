...
module.exports = router
  ...
  .put('/profile/friends/', ensureAuth, (req, res) => {
    ...
    return User.findOne(query)
      .populate('friends')
      .then(result => {
        if(!result.friends.includes(req.account.id) && (result.email !== query)) {
  ...
  .get('/profile/friends', ensureAuth, (req, res) => {
    User.findById(req.account.id)
      .populate('friends', 'firstName lastName pictureUrl email contact callOrText availability shareables')
